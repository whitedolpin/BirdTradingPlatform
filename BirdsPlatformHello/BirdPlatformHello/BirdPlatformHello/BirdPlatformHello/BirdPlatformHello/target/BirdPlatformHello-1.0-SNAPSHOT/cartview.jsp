            ...
            <c:if test="${sessionScope.allShopCart.getMutilShopCart().values()!=null}">
                <c:if test="${not empty sessionScope.allShopCart.getMutilShopCart().values()}">
                                        ...
                                        <form action="cart" method="POST">
                                            ...
                                            <div class="shop__product">
                                                ...
                                                <div class="shop__product--name">
                                                    ...
                                                    <input class="myDiv" type="checkbox" name="checkoutlist" value="${item.getProduct().getProductID()}"/>

                                                    <img src="${item.getProduct().getImg()}" alt="" />
                                                    <p>${item.getProduct().getProductName()}<strong style="margin-left: 20px;">${requestScope.message}</strong></p>
                                                </div>
                                                <div class="flex-dis">
                                                    <div class="shop__product--description">
                                                        ...
                                                        <p class="mg-top">$${item.getProduct().getPriceOut()*item.getProduct().getpSale()
                                                                             *item.getQuantity()}</p>
                                                        <div class="operation">
                                                            <a  href="cart?action=removeitem&productID=${item.getProduct().getProductID()}"><svg style="margin-bottom: 10px;"
                    ...
                    <section class="check-out">  
                        ...
                        <div style="margin-left: 73.7%" class="confirm-box">
                            <div class="ip-checkout">
                                <span>Number of products: <span id="TotalValueCount">0</span>  </span>
...
for (let i = 0; i < divElements.length; i++) {
  ...
  // Using closure to maintain the value of i
  (function (index) {
    divElement.addEventListener('click', function () {
      var sumElement = document.getElementById("TotalValueCount");
      var sum = parseInt(sumElement.innerText);
